...
import { STAGES, generateStageImages, type StageKey, type GeneratedImage } from "@/lib/services/aiService";
import { saveProject } from "@/lib/services/databaseService";
...
const PlaceholderCard: React.FC<{
  ...
  <button
    onClick={onClick}
    className={`relative rounded-xl overflow-hidden aspect-square transition-all duration-300 group w-full h-full ${
      selected
  ...
  >
    {image.imageUrl ? (
      ...
      <div className="w-full h-full bg-gradient-to-br from-muted to-muted/50 flex flex-col items-center justify-center gap-1.5 sm:gap-3 p-2 sm:p-4">
        ...
        <span className="text-xs sm:text-sm font-body text-foreground font-medium text-center leading-tight line-clamp-2">
          {image.label}
        </span>
        <span className="text-[10px] sm:text-xs text-muted-foreground font-body truncate">{image.stageName}</span>
      </div>
    )}

    {image.imageUrl && (
      <div className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-black/80 via-black/30 to-transparent p-2 text-left z-10">
        <span className="text-[10px] sm:text-xs font-body text-white line-clamp-1 leading-tight font-medium">
          {image.label}
        </span>
      </div>
    )}
    ...
    {selected && (
      <div className="absolute top-2 right-2 w-6 h-6 sm:w-7 sm:h-7 rounded-full bg-primary flex items-center justify-center z-10">
...
const StageWizard: React.FC<StageWizardProps> = ({ options, onClose }) => {
  ...
  const handleNext = () => {
    ...
    if (currentStageIdx < STAGES.length - 1) {
      ...
      // Paywall: Stage 1 (Entry) is free. Stage 2+ requires paid plan.
      if (nextIdx >= 1 && profile?.plan === "free") {
        setShowPaywall(true);
        return;
      }
  ...
  const handleSave = async () => {
    await saveProject({
      name: `Moodboard — ${new Date().toLocaleDateString("en-IN")}`,
      imageUrl: null,
      palette: [],
      options: options as unknown as Record<string, any>,
      stages: Object.fromEntries(
        Object.entries(stageSelections).map(([k, v]) => [k, v.label])
      ),
      logoUrl,
    });
  ...
  const handleDownload = (format: string) => {
    toast({ title: "Download Started", description: `Preparing ${format.toUpperCase()} file... (mock)` });
  };

  // ════════════════════════════════════════════
  // COMPLETED: Interactive Editorial Collage
  // ════════════════════════════════════════════
  if (completed) {
    return (
      <motion.div
      ...
      >
        <div className="text-center">
          ...
          <p className="text-muted-foreground font-body text-sm">
            Your curated 6-stage wedding vision · <span className="text-lovers-blush font-medium">Drag to rearrange</span>
          </p>
        </div>
        ...
        <div className="bg-white/40 dark:bg-black/30 backdrop-blur-xl border border-white/30 dark:border-gray-700/50 rounded-3xl p-5 shadow-xl shadow-black/5">
          <Reorder.Group
            ...
            className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 lg:grid-rows-3 gap-3 auto-rows-fr"
            style={{ minHeight: isMobile ? "auto" : "520px" }}
          >
            {collageItems.map((item, idx) => {
              // Editorial layout: varied cell sizes
              // Cell 0 → 2×2 hero, Cell 1 → 2×1, Cell 2 → 1×1, Cell 3 → 1×1, Cell 4 → 1×2, Cell 5 → 1×1
              const gridStyles: Record<number, React.CSSProperties> = isMobile
                ? {}
                : {
                    ...
                    4: { gridColumn: "1 / 3", gridRow: "3 / 4" },
                    5: { gridColumn: "3 / 5", gridRow: "3 / 4" },
                  };

              const isHero = !isMobile && idx === 0;
              ...
              return (
                ...
                >
                  <div className={`
                    ...
                    bg-gradient-to-br from-lovers-cream via-white/60 to-lovers-blush/10
                    dark:from-gray-800/60 dark:via-gray-900/50 dark:to-lovers-blush/5
                    ...
                    transition-all duration-300 flex flex-col items-center justify-center gap-3 p-4
                    relative
                  `}>
                    {/* Image if available */}
                    {item.imageUrl ? (
                      <img
                        src={item.imageUrl}
                        alt={item.selectionLabel}
                        className="absolute inset-0 w-full h-full object-cover transition-transform duration-500 group-hover:scale-105"
                      />
                    ) : (
                      /* Icon Placeholder if no image */
                      <div className={`
                        ${isHero ? "w-20 h-20" : "w-14 h-14"}
                        rounded-full bg-lovers-blush/10 flex items-center justify-center shrink-0
                      `}>
                    ...
                    {/* Drag Handle */}
                    <div className="absolute top-2.5 right-2.5 opacity-0 group-hover:opacity-60 transition-opacity z-10">
                      <GripVertical size={16} className={item.imageUrl ? "text-white" : "text-gray-400"} />
                    </div>

                    {/* Stage Badge */}
                    <div className={`
                      absolute top-2.5 left-2.5 z-10 px-3 py-1 rounded-full text-[10px] font-body font-semibold uppercase tracking-wider
                      ${item.imageUrl 
                        ? "bg-black/60 text-white border border-white/20 backdrop-blur-md" 
                        : "bg-lovers-gold/10 text-lovers-gold border border-lovers-gold/20"
                      }
                    `}>
                      {item.stageIcon} {item.stageLabel}
                    </div>

                    {/* Selection Text overlay if image, or center text if no image */}
                    {item.imageUrl ? (
                      <div className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-black/90 via-black/40 to-transparent p-4 pt-10 text-left z-10">
                        <span className={`
                          ${isHero ? "text-base" : "text-xs sm:text-sm"}
                          font-heading text-white line-clamp-1 leading-snug
                        `}>
                          {item.selectionLabel}
                        </span>
                      </div>
                      ...
                      <span className={`
                        ...
                        font-heading text-foreground text-center leading-snug
                      `}>
                        {item.selectionLabel}
                      </span>
                    )}
                    ...
                    <div className="absolute inset-0 rounded-2xl opacity-0 group-hover:opacity-100 transition-opacity duration-500 pointer-events-none bg-gradient-to-tr from-transparent via-white/10 to-transparent z-10" />
                  </div>
                </Reorder.Item>
              );
            })}
  ...
  return (
    <div className="w-full max-w-3xl mx-auto space-y-6">
      {/* Stage progress */}
      <div className="flex items-center justify-start lg:justify-center gap-1.5 overflow-x-auto no-scrollbar py-2 px-1 flex-nowrap w-full">
        {STAGES.map((stage, idx) => (
          <React.Fragment key={stage.key}>
            <div
              className={`flex items-center gap-1 px-2.5 py-1.5 rounded-full text-xs font-body transition-all shrink-0 ${
                idx === currentStageIdx
        ...
        <DialogContent className="bg-white/70 dark:bg-gray-900/70 backdrop-blur-2xl border border-white/30 dark:border-gray-700/50 shadow-2xl sm:max-w-md">
          <DialogHeader>
            ...
            <DialogDescription className="font-body">
              Stage 1 (Entry) is free! Upgrade to Pro to continue generating all 6 stages
