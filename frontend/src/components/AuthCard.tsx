...
import { Sparkles, Gift, Mail, Lock, ArrowRight } from "lucide-react";
      ...
      >
        ...
        <div className="text-center mb-8">
          ...
          <h1 className="font-heading text-3xl sm:text-4xl text-gray-800 dark:text-gray-100 tracking-tight">
            Welcome to{" "}
            <span className="text-gradient-gold">Lover&apos;s Ai</span>
          </h1>
          ...
          <div className="flex mb-7 bg-white/30 dark:bg-white/5 rounded-2xl p-1 border border-white/20 dark:border-white/5">
            {(["login", "signup"] as const).map((m) => (
              <button
                key={m}
                onClick={() => setMode(m)}
                className={`relative flex-1 py-2.5 text-sm font-body font-medium rounded-xl transition-all duration-300 ${
                  mode === m
